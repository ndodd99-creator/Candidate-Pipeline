export default async function handler(req, res) {
  ...
  const endpoint = req.query.endpoint || 'candidates';

  try {
    const response = await fetch(`https://${DOMAIN}/api/${SLUG}/${endpoint}`, {
      headers: {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: `Loxo API error: ${response.status}` });
    }
